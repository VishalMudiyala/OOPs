...
public class login{
	login(){
		...
		ImageIcon i = new ImageIcon("7.png");
			...
			public void actionPerformed(ActionEvent e) {
				...
				Statement stmt = null;
			      try {
			         ...
			         String sql = "select * from register where email='"+username+"' and password='"+password+"' and role='"+role+"'";
			         
			         ResultSet rs = stmt.executeQuery(sql);
			         
			         if(rs.next()) {
			        	 if(l.getSelectedItem()=="Company Manager") {
			        		 compman o = new compman();
			        	 }
			        	 if(l.getSelectedItem()=="Distributer") {
			        		 distributer o = new distributer();
			        	 }
			         }
